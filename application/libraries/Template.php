...
class Template {
	 ...
	 * no parsing. Used by the yield_content() and block() 
	 * 
	 * @var mixed
	 * @access public
	 */
	 ...
	 * @return void
	 */
	public function render($layout='', $cache_me=false, $return_html=false) 
	{	
		...
		$output = str_replace('{yield_content}', $this->yield_content(true), $output);	
	 ...
	 * @return void
	 */
	public function yield_content($bypass=false) 
	{ 
		$this->_mark('Template_yield_content_start');
		
		// If we've cached the layout, we don't return anything except the 
		// yield_content function itself.
		if ($bypass === true)
		{
	 ...
	 *
	 * Used by the yield_content and block methods.
	 *
	 * @access	private
	 * @return	boolean
	 */
