...
//exit;
     ...
     // $sWhere = substr_replace($sWhere, '', -3);
      ...
      // Individual column filtering
        $request['columns'] = $this->ci->input->post('columns');
        $column_filter = array();
        if ( isset( $request['columns'] ) ) {
                  for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                          ...
                          if ( $requestColumn['searchable'] == 'true' && $str != '' ) {
                                  ...
                                  if($this->check_cType()){
                                      $column_filter[$this->select[$mColArray[$i]['data']]] = $str ;
                                  }else{
                                      $column_filter[$this->select[$this->columns[$i]]] = $str ;
                                  }
                          }
                  }
          }
